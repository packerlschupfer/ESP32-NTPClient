...
void setup() {
    ...
    // A rejected server (list full, bad name) would otherwise leave the client
    // silently syncing against fewer sources than intended.
    if (!ntp.addServer("pool.ntp.org", 123)) {
        Serial.println("Failed to add pool.ntp.org");
    }
    if (!ntp.addServer("time.google.com", 123)) {
        Serial.println("Failed to add time.google.com");
    }
