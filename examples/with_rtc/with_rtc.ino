...
void setup() {
    ...
    // Set system time from RTC initially
    struct timeval tv;
    ...
    ntp.setRTCCallback([](time_t epoch) {
        ...
        if (rtc.setTime(dt)) {
            ...
            // Also update RTC temperature compensation if needed
    ...
    ntp.onSync([](const NTPClient::SyncResult& result) {
        if (result.success) {
            ...
            Serial.printf("  Offset: %ldms, RTT: %dms, Stratum: %d\n",
                         result.offsetMs, result.roundTripMs, result.stratum);
        } else {
