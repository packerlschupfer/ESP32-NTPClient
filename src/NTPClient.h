...
public:
    ...
    struct SyncResult {
        ...
        int32_t offsetMs;         // Time offset in milliseconds
        ...
        SyncResult() : syncTime(0), offsetMs(0), roundTripMs(0), stratum(0), success(false) {
            serverUsed[0] = '\0';
            error[0] = '\0';
        }
    };
...
private:
    ...
    time_t parseNTPPacket(const NTPPacket& packet, uint16_t& rtt);
    ...
    void applyTimeOffset(time_t newTime);
